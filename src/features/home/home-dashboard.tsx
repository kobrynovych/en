    ...
    <div className="space-y-6 pb-20 lg:pb-0">
      <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
        <div className="rounded-lg border border-slate-200 bg-white p-5 sm:p-7 dark:border-slate-700 dark:bg-slate-800/60">
          ...
          <h1 className="mt-5 max-w-3xl text-3xl font-black leading-tight text-slate-950 sm:text-5xl dark:text-slate-100">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        {stats.levels.map((level) => (
          <Link
            ...
            className="rounded-lg border border-slate-200 bg-white p-5 transition hover:-translate-y-0.5 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-600 dark:border-slate-700 dark:bg-slate-800/60 dark:hover:bg-slate-800"
          >
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-sm font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">Рівень</p>
                <h2 className="mt-1 text-3xl font-black text-slate-950 dark:text-slate-100">{level.level}</h2>
              </div>
              <span className="grid size-12 place-items-center rounded-md bg-emerald-50 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400">
            ...
            {stats.categories.slice(0, 6).map((category) => (
              <div key={category.category}>
                <div className="mb-1 flex items-center justify-between text-sm">
                  <span className="font-semibold capitalize text-slate-800 dark:text-slate-200">{category.category}</span>
                  <span className="text-slate-500 dark:text-slate-400">{category.progressPercent}%</span>
                </div>
...
function Metric({ icon, label, value }: { icon: ReactNode; label: string; value: string }) {
  return (
    <div className="rounded-md border border-slate-200 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-800/80">
      <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400">
        {icon}
        <span className="text-xs font-bold uppercase tracking-wide">{label}</span>
      </div>
      <p className="mt-2 text-xl font-black text-slate-950 dark:text-slate-100">{value}</p>
    </div>
  );
}
...
function ModeLink({ href, icon, label }: { href: string; icon: ReactNode; label: string }) {
  return (
    <Link
      href={href}
      className="flex min-h-28 flex-col justify-between rounded-md border border-slate-200 bg-slate-50 p-4 font-bold text-slate-950 transition hover:bg-white hover:shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-600 dark:border-slate-700 dark:bg-slate-800/60 dark:text-slate-100 dark:hover:bg-slate-700"
    >
